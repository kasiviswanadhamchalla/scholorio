package com.scholario.lending.config;

import com.scholario.lending.security.JwtAuthenticationFilter;
import com.scholario.lending.security.KeycloakJwtAuthenticationConverter;
import com.scholario.lending.security.UnassignedRoleFilter;
...
public class SecurityConfig {
    ...
    public JwtDecoder jwtDecoder() {
        if (!keycloakEnabled) {
            log.info("Keycloak authentication is explicitly disabled in Lending Service.");
            return null;
        }
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers(
                    ...
                    "/graphql",
                    "/graphiql/**",
                    "/internal/**" // Allow internal REST controller access (you can lock this down if needed)
                ).permitAll()
    ...
    private BearerTokenResolver customBearerTokenResolver() {
        ...
        return request -> {
            ...
            if (token != null) {
                // If it can be parsed as a symmetric JWT locally, don't pass it to the Keycloak resource server filter
                try {
                    com.auth0.jwt.JWT.decode(token);
                    // Check if signature algorithm is HMAC256 (symmetric secret)
                    ...
                    if ("HS256".equalsIgnoreCase(alg)) {
                        ...
                        return null; // Return null so keycloak filter ignores it, letting JwtAuthenticationFilter process it
                    }
                } catch (Exception e) {
